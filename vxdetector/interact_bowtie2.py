...
def buildbowtie2(path):
    ...
    input_ref = f'{path}Indexed_bt2/85_otus.fasta'  # Path to the reference genome
...
def mapbowtie2(fasta_file, read2_file, path, temp_path, paired, bowtie2_params=None):    
    ...
    # Handle unpaired reads
        ...
        aligned_path = f'{temp_path}unpaired.bed'  # Output path for unpaired reads
        bowtie2_base_cmd += ['-U', fasta_file]
        
        # Commands for Samtools and Bedtools
        cmd_samtools = [samtools_path, 'view', '-b', '-q', '30', '-S', '-F', '4']
        cmd_bedtools = [bedtools_path, 'bamtobed', '-i', 'stdin']

        # Run the Bowtie2 and Samtools pipeline
        with open(log_path, 'w') as log_file, open(aligned_path, 'w') as output_file:
            bowtie_process = subprocess.Popen(bowtie2_base_cmd, stderr=log_file, stdout=subprocess.PIPE)
            samtools_process = subprocess.Popen(cmd_samtools, stdin=bowtie_process.stdout, stdout=subprocess.PIPE)
            subprocess.run(cmd_bedtools, stdin=samtools_process.stdout, stdout=output_file)
    ...
    return aligned_path, Error  # Return the path to the aligned file and error status
